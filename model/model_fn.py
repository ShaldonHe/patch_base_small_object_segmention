...
from model.Nets import patch_multi_task as m_patch
...
def patch_multitask_fn(features, labels, mode, params):
    predict = m_patch(features["images"], mode == tf.estimator.ModeKeys.TRAIN)
    ...
    def class_loss(label,class_result):
        print(tf.reduce_sum(label,axis=(1,2)).shape)
        print(class_result.shape)
        return tf.losses.mean_squared_error(tf.reduce_sum(label,axis=(1,2)),class_result)
    ...
    if labels is not None:
        ...
        # loss = tf.losses.mean_squared_error(labels,predict)#iou_loss(labels,predict)
        # loss = tf.reduce_sum(tf.sqrt( tf.abs(labels-predict)))
        loss = class_loss(labels,class_result)
        train_op = optimizer.minimize(loss=loss, global_step=tf.train.get_global_step())
    ...
    eval_metric_ops = {'label':labels,'predict':predict,'data':features['images']}
...
def patch_classification_fn(features, labels, mode, params):
    predict = m_patch(features["images"], mode == tf.estimator.ModeKeys.TRAIN)
    class_result=predict['class_result']
    optimizer = tf.train.GradientDescentOptimizer(learning_rate=params['learning_rate'])
    def class_loss(label,class_result):
